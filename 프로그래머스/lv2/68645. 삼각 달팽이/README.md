...
모든 조건문마다 arr[x][y] += 1과 cnt -= 1을 넣어줬는데 조건문 안에서는 x, y값만 변경하고 나중에 arr[x][y] += 1과 cnt -= 1를 처리하면 코드량이 줄어들 수 있었다.

또한 sum(고차원 리스트, [])를 통해 차원 하나를 축소하고 병합하는 기능을 사용할 수 있다는걸 배웠다 끗
