...
🎃회고
Idea
여분의 체육복이 있는 경우도 도난당하게 된다면 빌려줄 수가 없으므로 lost와 reserve에서 서로 겹치는건 뺀다
그 후 체육복이 있는 인덱스는 True로 처리 후 여분의 학생이 앞 뒤를 체크하며 빌려주면 된다
그러나 먼저 좌쯕체크 후 우측을 체크하는거라 reserve 학생이 정렬되지 않은 채 들어오면 예외 케이스가 발생한다
예를 들어 아래와 같은 상황이다
n = 5
lost = [2, 4]
reserve = [3, 1]
