...
몫, 나머지의 순서를 헷갈려서 1차 실수
순위를 정하는 과정에서 아이디어를 생각했는데 딕셔너리로 하는 것 까지는 좋았으나 정렬에서 문제가 생김 ㅜ.ㅜ

예를 들어,
cnt = {"2": 0, "3": 2, "1": 2}
tmp = sorted(cnt.items(), key=lambda x: (x[1], -x[0]), reverse=True)
와 같이 정렬을 하려고 하면 str은 음수취급이 안되서 정렬이 안됨! ㅜㅜ 내부 알고리즘이 그냥 역순으로 취급이 아니라 실제로 음수를 붙인 후 거기에 대해 정렬이였음
